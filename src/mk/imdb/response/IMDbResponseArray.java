...
public class IMDbResponseArray extends IMDbResponse {
	 ...
	 */
	public IMDbResponseArray(JSONObject json) {
		super(json);
		
		if (!hasError()) {
			
			if (json.has(IMDbConstants.TOTAL_RESULTS)) {
				setResults(json.getInt(IMDbConstants.TOTAL_RESULTS));
			} else {
				setResults(1);
			}
		
			if (json.has(IMDbConstants.RESULT)) {
				setData(json.getJSONArray(IMDbConstants.RESULT));
			} else {
	 ...
	 */
	public IMDbResponseArray(IMDbStatus status) {
		super(status);
	}
